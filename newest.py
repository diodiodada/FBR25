...
def create_backward_model():
    ...
    B_action_output = Dense(4, name='B_action_output')(B_input_4)
...
def add_one_cell(F, B, R, input_for_25_nets):
    ...
    if pure_or_not != 'none':
        pure_output.append(node_return)
    else:
...
def construct_the_whole_network():
    ...
    # define the model
    model_for_25_nets = Model(inputs=input_for_25_nets, outputs=output_24)
...
def train(model_for_25_nets):
    ...
    model_for_25_nets.fit([state_feed,action_feed,next_state_feed],
                          [state_feed] * 8 + [action_feed] * 8 + [next_state_feed] * 8,
                            batch_size=50,
                            epochs=100,
                            verbose=1,
